...
struct MatterSession *session_init();
void session_free(struct MatterSession *);
void session_reset(struct MatterSession *ctx);

int handle_btp_packet(struct MatterSession *ctx, const uint8_t *req, int reqsize);
uint16_t next_btp_packet_to_send(struct MatterSession *ctx, uint8_t **data);
int8_t get_network_state(struct MatterSession *ctx);
